...
async function generatePDF() {
	...
	// Scrape la prod - changez l'URL ici
	const urlToScrape = "https://www.groupemagellim.com/reim/viagenerations-test-pdf/";
	//const urlToScrape = "https://www.groupemagellim.com/reim/scpi-foncieres-des-praticiens/";
	...
	// ETAPE 2 : Scrape les graphiques (sur une autre page si nécessaire)
	// Si les graphiques sont sur la même page, on les récupère maintenant
	// Sinon, navigue vers la page qui contient les graphiques

	// Attend que les graphiques Chart.js soient rendus
	...
	try {
		...
		// Ajoute les graphiques aux données
	...
	// Sauvegarde les données dans un fichier JSON (pour Live Server)
	const fs = require("fs");
	fs.writeFileSync(__dirname + "/data.json", JSON.stringify(data, null, 2));
	console.log("✅ Données scrapées et sauvegardées dans data.json");

	// Va sur le template local
	await page.goto("file://" + __dirname + "/template-magellim.html");
	...
	const fileName = `${topSectionTitle} - Reporting du ${performanceTitle}.pdf`.replace(/[/\\?%*:|"<>]/g, "-");
